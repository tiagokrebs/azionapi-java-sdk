...
public class WAFEvents401 {
  ...
  * @throws IOException if the JSON Object is invalid with respect to WAFEvents401
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!WAFEvents401.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!WAFEvents401.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `WAFEvents401` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
