...
public class ApplicationCacheResponseDetails {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ApplicationCacheResponseDetails
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ApplicationCacheResponseDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ApplicationCacheResponseDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ApplicationCacheResponseDetails` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
