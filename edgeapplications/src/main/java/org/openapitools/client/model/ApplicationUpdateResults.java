...
public class ApplicationUpdateResults {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ApplicationUpdateResults
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ApplicationUpdateResults.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ApplicationUpdateResults.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ApplicationUpdateResults` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
