...
public class DeviceGroupsResponseLinks {
  ...
  * @throws IOException if the JSON Object is invalid with respect to DeviceGroupsResponseLinks
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DeviceGroupsResponseLinks.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DeviceGroupsResponseLinks.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DeviceGroupsResponseLinks` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
