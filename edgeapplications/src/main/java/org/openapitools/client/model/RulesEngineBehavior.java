...
public class RulesEngineBehavior {
  ...
  * @throws IOException if the JSON Element is invalid with respect to RulesEngineBehavior
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RulesEngineBehavior.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!RulesEngineBehavior.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RulesEngineBehavior` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("target") != null && !jsonObj.get("target").isJsonNull()) {
        RulesEngineBehaviorTarget.validateJsonElement(jsonObj.get("target"));
      }
  }
