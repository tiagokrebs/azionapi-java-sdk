...
public class RulesEngineResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to RulesEngineResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RulesEngineResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RulesEngineResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RulesEngineResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      OriginsResponseLinks.validateJsonElement(jsonObj.get("links"));
      ...
      for (int i = 0; i < jsonArrayresults.size(); i++) {
        RulesEngineResultResponse.validateJsonElement(jsonArrayresults.get(i));
      };
  }
