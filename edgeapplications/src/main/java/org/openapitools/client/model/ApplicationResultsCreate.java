...
public class ApplicationResultsCreate {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ApplicationResultsCreate
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ApplicationResultsCreate.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ApplicationResultsCreate.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ApplicationResultsCreate` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
