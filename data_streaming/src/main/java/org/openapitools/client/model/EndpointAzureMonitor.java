...
public class EndpointAzureMonitor {
  ...
  * @throws IOException if the JSON Element is invalid with respect to EndpointAzureMonitor
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EndpointAzureMonitor.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!EndpointAzureMonitor.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EndpointAzureMonitor` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
