...
public class EndpoinrtS3 {
  ...
  * @throws IOException if the JSON Element is invalid with respect to EndpoinrtS3
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!EndpoinrtS3.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!EndpoinrtS3.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `EndpoinrtS3` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
